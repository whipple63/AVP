...
// Added by Tony to include the dump in the log file
#include "sdLogger.h"
extern sdLogger sd;
...
// Added by Tony to include the dump in the log file
void CApplicationMonitor::DumpToLogFile(bool bOnlyIfPresent) const
{
  CApplicationMonitorHeader Header;
  CCrashReport Report;
  uint8_t uReport;
  ...
  if (!bOnlyIfPresent || Header.m_uSavedReports != 0)
  {
    sd.println(sd.getSerFileName(), NULL, F("Application Monitor"), true);
    sd.println(sd.getSerFileName(), NULL, F("-------------------"), true);
    sd.println(sd.getSerFileName(), NULL, String(F("Saved reports: ")) + Header.m_uSavedReports, true);
    sd.println(sd.getSerFileName(), NULL, String(F("Next report: ")) + Header.m_uNextReport, true);

    for (uReport = 0; uReport < Header.m_uSavedReports; ++uReport)
    {
      LoadReport(uReport, Report);

      sd.print(sd.getSerFileName(), NULL, uReport, DEC, true);
      uAddress = 0;
      memcpy(&uAddress, Report.m_auAddress, PROGRAM_COUNTER_SIZE);
      sd.print(sd.getSerFileName(), NULL, F(": word-address=0x"), false);
      sd.print(sd.getSerFileName(), NULL, uAddress, HEX, false);
      sd.print(sd.getSerFileName(), NULL, F(": byte-address=0x"), false);
      sd.print(sd.getSerFileName(), NULL, uAddress * 2, HEX, false);
      sd.print(sd.getSerFileName(), NULL, F(", data=0x"), false);
      sd.println(sd.getSerFileName(), NULL, Report.m_uData, HEX, false);
    }
  }
}
