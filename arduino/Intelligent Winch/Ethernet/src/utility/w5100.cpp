...
//
// CODE ADDED to standard w5100 class to send keepalive messages to test if the link is still good
//
uint8_t W5100Class::send_keep_alive(SOCKET s) {
  ...
  // Could potentially check if the socket becomes closed and if so
  // check if the reason is a timeout.  This is unnecessary though becuase
  // if the keepalive fails, the socket will be closed.  We don't really
  // need to know why.
  
  return(0);  // return code has become meaningless.
} //END OF DEBUG
