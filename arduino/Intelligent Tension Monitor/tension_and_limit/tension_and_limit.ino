/*
  ...
  This version modified for the Smart Tension 3.1 board
...
const int noLowerOutPin = 7;  // logic low is activated
const int noRaiseOutPin = 6;   // logic low is activated
...
const int limitInHi = 9;     // configured with internal pullup resistor - switch to ground to activate
...
int minVal = 145;    // min and max possible values
int maxVal = 1584;
int hiThresh = 750;  // above which the high output is turned on
...
//  Serial.println( "");
  ...
  Serial.print(F("Max values for hi and low are currently set to: "));
  Serial.print( maxVal );
  Serial.print(F(", "));
  ...
  // print a header for the serial output values
  Serial.println("");
  Serial.println(F("Raw   = Raw A/D Values 0-2048"));
  Serial.println(F("% Lim = Percent of set limit values"));
  Serial.println(F("% Max = Percent of max values (full range of sensor)"));
  Serial.println(F("NR    = No Raise (0:Off, 1:On)"));
  Serial.println(F("NL    = No Lower (0:Off, 1:On)"));
  Serial.println(F("HS    = High Limit Switch (0:Off, 1:On)"));
  Serial.println(F("LS    = Low Limit Switch (0:Off, 1:On)"));
  Serial.println(F("HT    = High Tension Threshold (before tension delay) (0:Off, 1:On)"));
  Serial.println(F("LT    = Low Tension Threshold (before tension delay) (0:Off, 1:On)"));
  Serial.println("");
  Serial.println(F(" Raw,  % Lim,  % Max, NR,NL,HS,LS,HT,LT"));
}
...
void loop() {  
  ...
  // if we are within 10% of the max value, blink the leds
  if (sensorValue > maxVal - 0.1*maxVal) {
    bar.blinkRate(1);
  }
  else {
    bar.blinkRate(0);
  ...
  if ( (millis() - msStartTime) >= 1000) {
    ...
    char pbuf[80], b1[10], b2[10];
    dtostrf(100.0*((float)(sensorValue - loThresh) / (hiThresh - loThresh)), 6, 1, b1);
    dtostrf(100.0*((float)(sensorValue - minVal) / (maxVal - minVal)), 6, 1, b2);
    snprintf(pbuf, 80, "%4d, %s, %s,  %1d, %1d, %1d, %1d, %1d, %1d",
      sensorValue, 
      b1,     // print out the value as percentage of operating range
      b2,     // print out the value as percentage of total range
      !digitalRead(noRaiseOutPin),
      !digitalRead(noLowerOutPin),
      !limitHi,
      !limitLo,
      sensorValue > hiThresh,
      sensorValue < loThresh
      );
    Serial.println(pbuf);  // finally send the line
