...
/*        this.load_gps = function() {
    ...
    function updateDataDisplay() {
        ...
        addRow(tbl, "last_wind_time", _appData.windReadings[0].time);
        ...
        addRow(tbl, "last_gps_time", _appData.gpsReadings[0].time);
...
<i>This page shows realtime data acquired from AVP3.  Refresh the page for updated data.<p>Currently, it displays the most recent GPS and Wind sensor readings.</i>
</div>
    <div id="data"></div>
    <div id="map3d"></div>
  ...
  $link = pg_Connect("host=eddy.ims.unc.edu dbname=avp user=postgres password=sonde");
	if($link == "") {
		echo "<B>Failed to connect to eddy</b><br>";
	}
  ...
  $numrows = pg_numrows($result);
   ...
   for($ri = 0; $ri < $numrows; $ri++) {
    ...
    echo "</wind>";
  $result = pg_exec($link, "select * from stones_gps order by \"time\" desc limit 1 offset 0");
  $numrows = pg_numrows($result);
    echo "<gps>";
   // Loop on rows in the result set.
   for($ri = 0; $ri < $numrows; $ri++) {
