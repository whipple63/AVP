...
<html>
	...
	<title>New River Autonomous Vertical Profilers</title>
...
//			updateDataDisplay();
		...
		function updateProfileDisplay() {
			$(document).ready(function(){
				...
				$('#profile_cal').datepicker({
					minDate: new Date(2008, 6, 14),
					maxDate: yesterday,
					changeMonth: true,
					changeYear: true,
					onSelect: function(dateText, inst) { drawProfilePlot(); }
				});
				$('#profile_cal').datepicker( "setDate", yesterday );
				drawProfilePlot();
			});
		...
		function drawProfilePlot() {
			...
			plotFile = "Morgan_Bay/" + "AVP2_" + selDate.getFullYear() + pad(selDate.getMonth()+1) + pad(selDate.getDate()) + ".png";
...
<body onload="on_page_load()">
	<h1>New River Autonomous Vertical Profilers</h1>
	<p>The Autonomous Vertical Profilers (AVPs) use a YSI 6600 Multi-Parameter Sonde to measure water temperature, 
	...
	<h2><a href="avp_monitor.php">Stones Bay</a> --- <a href="morgan_bay.php">Morgan Bay</a></h2>
	
	<div id="pic"><img id="avp_image" src="Morgan_AVP.png"></div>
	
	<div id="header">
	<h2>Morgan Bay Profiler - Recent Conditions</h2>
	<p>This page shows data acquired from the station located in Morgan Bay, New River, NC.  Data are available daily
	for the prior day.  Profile images are updated every night and can be reviewed by date using the calendar below.</p>
	...
	<?php
		 ...
		 */
		$link = pg_Connect("host=wave.ims.unc.edu dbname=avp user=ims password=ims6841");
		if($link == "") {
			echo "<B>Failed to connect to database</b><br>";
		}
		
		$result = pg_exec($link, "select * from stones_wind order by \"sample_time\" desc limit 1 offset 0");
		if($result == "") {
			echo "<B>Failed to search stones_wind</b><br>";
		}		
		...
		$result = pg_exec($link, "select * from stones_gps order by \"sample_time\" desc limit 1 offset 0");
		if($result == "") {
			echo "<B>Failed to search stones_gps</b><br>";
		}		
		$numrows = pg_numrows($result);
		echo "\n\t<gps ";
		// Loop on rows in the result set.
		...
		$result = pg_exec($link, "select * from stones_depth order by \"sample_time\" desc limit 1 offset 0");
		if($result == "") {
			echo "<B>Failed to search stones_depth</b><br>";
		}		
		$numrows = pg_numrows($result);
		echo "\n\t<depth ";
		// Loop on rows in the result set.
