...
<html>
	...
	<script>
		...
		function drawProfilePlot() {
			...
			plotFile = "jlnewhope/" + "JLNewHope_" + selDate.getFullYear() + pad(selDate.getMonth()+1) + pad(selDate.getDate()) + ".png";
...
<h1>Jordan Lake Autonomous Vertical Profilers</h1>
	...
	<p>This page shows data acquired from the station located in the New Hope Creek section of B. Everett Jordan Reservoir.  Data are available in
	...
	<?php
		 ...
		 */
		$link = pg_Connect("host=storm.ims.unc.edu dbname=jlnewhope user=ims password=ims6841");
		if($link == "") {
			echo "<B>Failed to connect to database</b><br>";
		}
		
		$result = pg_exec($link, "select * from jlnewhope_wind order by \"sample_time\" desc limit 1 offset 0");
		if($result == "") {
			echo "<B>Failed to search jlnewhope_wind</b><br>";
		}		
		...
		$result = pg_exec($link, "select * from jlnewhope_gps order by \"sample_time\" desc limit 1 offset 0");
		if($result == "") {
			echo "<B>Failed to search jlnewhope_gps</b><br>";
		}		
		$numrows = pg_numrows($result);
		echo "\n\t<gps ";
		// Loop on rows in the result set.
		...
		$result = pg_exec($link, "select * from jlnewhope_depth order by \"sample_time\" desc limit 1 offset 0");
		if($result == "") {
			echo "<B>Failed to search jlnewhope_depth</b><br>";
		}		
		$numrows = pg_numrows($result);
		echo "\n\t<depth ";
		// Loop on rows in the result set.
