...
#-------------------------------------------------------------------------------
# Name:        winch.py
# Purpose:     Contains class for Autonomous Vertical Profiler Winch
#              Integrates data from a sonde and motor broker clients.
#              Examples:
#
# Author:      neve
#
...
class Winch(object):
    ...
    REQUIRED_MM3_SUBSCRIPTIONS = ('current_limit','desired_position',
                                  ...
                                  'temp_fault')
    DESIRED_SONDE_SUBSCRIPTIONS = ('depth_m','spcond_mScm','sampling')
    DESIRED_AIO_SUBSCRIPTIONS = ('voltage_ADC',) # Alias for adc_
    ...
    def __init__(self,context,**kwargs):
        ...
        if context.sonde.__class__ == None.__class__:
    ...
    def load_config(self,reload_config=False,**kwargs):
        ...
        self.CABLE_LENGTH       = float(w_config.get('CABLE_LENGTH',12.0)) # Not used yet
        self.ZERO_POS_MAX_ERROR = int(w_config.get('ZERO_POS_MAX_ERROR',10000))
        CLICKS_PER_REVOLUTION   = int(w_config.get('CLICKS_PER_REVOLUTION',64000)) #
        DRUM_DIAMETER           = float(w_config.get('DRUM_DIAMETER',0.32)) #
        self.CLICKS_PER_METER   = CLICKS_PER_REVOLUTION/(DRUM_DIAMETER * pi) 
        return
    def _add_subscriptions(self,instrument_object,subscription_list,**kwargs):
        ...
        if len(new_subscriptions) > 0:
            ...
            instrument_object.add_subscriptions(new_subscriptions,on_change=True,subscriber=instrument_object.BROKER_NAME,**kwargs)
        self.added_subscriptions.update({instrument_object:new_subscriptions})
        return
    def _done(self):
        ''' Called whenb we are done with the winch object.
        Does some unsubscribing
        '''
        for instrument_object,subscription_list in self.added_subscriptions.items():
    ...
    def move_to_absolute(self,location,speedlimit=None,amps_limit=None,in_water=None,in_pos=False,stop_funct=None,stop_args=None,**kwargs):
        ...
        if result['move_to_position'].get('error',{}).get('message',False):
            ...
            result['monitor_move'] = self.monitor_move(max_stopped=timedelta(seconds=30),# These are all the defaults and optional
                                                       ...
                                                       in_pos=in_pos,
                                                       in_pos_max_time=timedelta(seconds=1),
                                                       debug_mode=debug_mode)
    ...
    def move_at_speed(self,motorspeed,direction=None,amps_limit=None,in_water=None,
                      stop_funct=None,stop_args=None,limit_stop=True,**kwargs):
        '''
        ...
        try:
            ...
            result['monitor_move'] = self.monitor_move(in_pos=True,
                                                       in_pos_max_time=timedelta(seconds=1),
                                                       stop_funct=stop_funct,
                                                       stop_args=stop_args,
                                                       limit_stop=limit_stop,
    ...
    def calibration(self,do_WL=True,end_pos=0,**kwargs):
        ...
        if result['stop_code'] in fatal_faults: #Make sure it didn't go badly.
            ...
            return result
        iters = 0
        while not self.mm3.neg_limit_switch.value: # If we didn't trip the upper  switch, raise some more
            print "Not at top, raising another 10000"
            result['move_to_relative.{0}'.format(iters)] = self.move_to_relative(park_step,
                                                                                 speedlimit=self.SLOW_SPEED,
                                                                                 max_stopped=timedelta(seconds=1),
        ...
        if self.sonde.in_water(instrumnet='any',debug_mode=debug_mode)[0] is False:
            ...
            self.logger.warning("failed to calibrate sonde position. ({0})".format(**kwargs))
            return result
        if do_WL is True:
    ...
    def park(self,park_location=0,speedlimit=None,get_surface_cond=False,in_pos=False,**kwargs):
        ...
        slowly so as not to stress the sonde bail or the lifting line.
        ...
        if self.mm3.position.value > self.SLOWDOWN_POSITION: # We go fast if we are deep
            ...
            # Stop if waterline detected, this means we ahve gone much too far.
        ...
        if speedlimit is None:
            ...
            speedlimit = int(speedlimit)
        if get_surface_cond is True: # If flag is set, stop at surface on the way up and get conductivity
            result['winch.move_to_absolute (surface)'] = self.move_to_absolute(location=park_location,
                                                                               speedlimit=speedlimit,
                                                                               amps_limit=None,
                                                                               in_water=True,
                                                                               in_pos=in_pos, # We will be stopping before we get to our destination
                                                                               stop_funct=self.press_wl,
                                                                               stop_args=[1], # Moving up
                                                                               debug_mode=debug_mode)
            sonde_in_water,result['sonde.in_water'] = self.sonde.in_water(instrument='all')
            if sonde_in_water is True:
                result['surface_spcond'] = self.sonde.spcond_mScm.value
                self.logger.info(
                    'paused at {0} {1} to get surface conductivity of {2} {3}'.format(
                        self.sonde.depth_m.value,
                         self.sonde.depth_m.units,
                         self.sonde.spcond_mScm.value,
                         self.sonde.spcond_mScm.units))
            else:
        ...
        if not self.mm3.in_position:
            ...
            if self.mm3.neg_limit_switch: # If we tripped the photo eye limit switch....
                ...
                if self.mm3.position < (self.ZERO_POS_MAX_ERROR*2) and self.sonde.conductivity < self.sonde.inwater_cond:
    ...
    def monitor_move(self,max_stopped=timedelta(seconds=30),
        ...
        '''
        This function monitors values from the sonde and motor controler.
        ...
        Keyword Arguments:
            ...
            ipc_poll         -- Poll ipc_conn database for external notifications. False only for passive monitoing
            debug_mode       -- 
        ...
        if stop_funct is not null:
            self.logger.debug("monitor_move using stop_function {0} with arguments {1}".format(stop_funct.func_name,stop_args))
        ...
        old_ts = datetime(2000,01,01).replace(tzinfo=pytz.reference.LocalTimezone()) # Way in the past so it fails the first comparison
        # Some database stuff
        ...
        while True: #This loop runs until the motor stops for some reason and we break out of it.
            ...
            if new_ts > old_ts: # we have new data from the mm3, so  do some checks
                ...
                if abs(self.mm3.velocity.value) > self.mm3.MIN_SPEED: # Are we moving very slowly or not at all?
                    ...
                    stop_reason['stop_desc'] = 'Velocity == 0 for {0} sec. (>{1})'.format(datetime.now(pytz.reference.LocalTimezone()) - zero_v_time,max_zero_v)
                if last_position != self.mm3.position.value: #We've moved
                    ...
                    stop_reason['stop_desc'] = 'Movement stopped for {0} sec. (>{1})'.format(datetime.now(pytz.reference.LocalTimezone()) - same_pos_time,max_zero_v)
                if in_pos is True and iterations >= 20:
                    ...
                    if position_delta <= self.POSITION_DB:
                        stop_reason['stop_code'] = 'in_position'
                        stop_reason['stop_desc'] = 'Position {0} within {1} of {2}'.format(self.mm3.position.value,self.POSITION_DB,desired_position)
            else: # no new values
    ...
    def feedback_header(self,show_legend=True,top=True,**kwargs):
        ...
        '''
        #debug_mode = kwargs.get('debug_mode',False)
        header = "  Winch                                 |Motor           |Sonde       |Sample Time\n    pos    trgt spd  PWM A B C D EF G H |amps/ lim@volts |depth cond  |HH:mm:ss.ii"
        if show_legend is True:
            legend =  " A=InPos,B=Photo,C=Tension,D=VLimited,EF=VLimit(+-), G=PWM Limited, H=Current@limit"
            CR = "\n"
            if top is True:
    ...
    def _feedback(self,new_ts):
        ...
        '''
        diagnostic = "{0:7}/{1:7}{2:4}{3:5}{4:2}{5:2}{6:2}{7:2}{8:2}{9:1}{10:2}{11:2} ".format(
                    ...
                    ' ',      #E
                    ' ',      #F
                    self.mm3.PWM_limited.value,            #G
                    self.mm3.current_limit.value)          #H
        try:
            depth_m = self.sonde.depth_m.value
        except Exception, e:
            depth_m = "?"
        try:
            spcond_mScm = self.sonde.spcond_mScm.value
        except Exception, e:
            spcond_mScm = "?"
        ...
        diagnostic += "|{0:6}{1:6}".format(depth_m,spcond_mScm)
    ...
    def cond_wl(self,raising):
        ...
        raising=1 for raising and 0 for lowering
        '''
        return self._check_wl(raising,instrument='spcond_mScm')
        ...
        if in_water is None: # We need to figure it out
            ...
            try:
                if up:
                    ...
                    result = self.sonde.in_water(instrument='spcond_mScm')
                    if debug_mode: print "spcond_mScm result {0}".format(result)
                    if not result[0]:
                        in_water = False
            except:
    ...
    def _newest_ts(self,**kwargs):
        ...
        '''
        try:
            return max(self.mm3.sample_time.sample_time,self.sonde.sample_time.sample_time)
        except:
            return self.mm3.sample_time.sample_time
