...
'''
FORKED ON 20131002
This module provides a class for accessing the Pertelian or ColdTears USB LCD through the pertd2 daemon.
...
from __future__ import print_function #Let's use the new print function
...
class LCDUserInterface(object):
    ...
    def insert(self,line_num,display_string,direction,delay=0):
        ...
        if line_num in self.VALID_LINES:
            if direction == 'up':
                scroll_lines = range(2,line_num + 1)
                self._shift_up(scroll_lines,delay)
            elif direction == 'down':
                scroll_lines = range(len(self.VALID_LINES) -1,line_num - 1,-1)
...
class Pertelian(LCDUserInterface):
    ...
    def stop_pertd2(self):
        ...
        return 1
    def write_line(self,line,display_string,delay=0,hold_open=0,color=None):
...
class ColdTears(LCDUserInterface):
