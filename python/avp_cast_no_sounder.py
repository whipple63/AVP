...
class Cast(object):
    ...
    def aio_setup(self,**kwargs):
        ...
        power_status = {}
        power_status['sonde'] = self.aio.power_sonde.value
        power_status['sounder'] = self.aio.power_sounder.value
        power_status['lisst'] = self.aio.power_LISST.value
        power_status['isco'] = self.aio.power_ISCO.value
        print "Instrument power status: {0}".format(power_status)
        if self.isco_cast is True: # Make sure ISCO is on
            if self.aio.power_ISCO.value != 1: # Use aio since uniquely among brokers, the ISCO broker doesn't turn itself on with a resume
                self.aio.power_ISCO.value = 1
                self.logger.debug('Turning on ISCO')
        elif self.lisst_cast is True: # Turn off ISCO
            if self.aio.power_ISCO.value == 1: # Use aio since uniquely among brokers, the ISCO broker doesn't turn itself on with a resume
                self.aio.power_ISCO.value = 0
                self.logger.debug('Turning on ISCO')
    ...
    def broker_setup(self,interactive=False,**kwargs):
        ...
        needed_objects = ['aio','gps','mm3','sonde']
        if self.lisst_cast is True:
        ...
        if self.adjusted_spcond is not None:
            message = 'Adjusting'
            if int(self.sonde.inwater_cond) > int(self.adjusted_spcond):
                message = 'Lowering'
            elif int(self.sonde.inwater_cond) < int(self.adjusted_spcond):
                message = 'Raising'
            else:
                self.adjuted_spcond = int(self.sonde.inwater_cond)
    ...
    def pre_position(self,**kwargs):
        ...
        result['02 move_to_relative(WL)'] = self.winch.move_to_relative(rel_distance,
                                                                        speed_limit=self.winch.MED_SPEED,
        ...
        if self.wipe: 
			...
			time.sleep(1)     # added by Tony because self.sampling.value is not up to date yet
    ...
    def post_cast_park(self,**kwargs):
        ...
        if self.park_pos not in self.POST_CAST_OPTIONS.keys():
            self.logger.warning("Invalid park_pos value {0}. Must be in {1}. Using 'default'".format(self.park_pos,self.POST_CAST_OPTIONS.keys()))
    ...
    def finish_cast(self,**kwargs):
        ...
        if self.lisst_cast is True or self.lisst_cast is 'aborted':
            ...
            if self.aio.power_LISST.value == 1:
        ...
        if self.isco_cast is True:
            ...
            result['15 aio.get_token'] = self.aio.get_token(calling_obj=self.__class__.__name__,
                                                         ...
                                                         override=True, # Force acquire
                                                         debug_mode=self.debug_mode)
            self.aio.power_ISCO.value = 0 # Turn it off
