...
class MainConsole(_BaseConsole):
    ...
    def setup_intro(self):
        ...
        self.intro += "|     {0:<68}     |{cr}".format(line,cr=CR)
        consoles = ('aio','powermon','flow','gps','sonde','wind')
        for c in consoles:
    ...
    def help_para(self):
        ...
        print("     usage: para [aio] [powermon] [flow] [gps] [isco] [sonde|s] [wind]")
    def do_exit(self,args):
...
class AIOConsole(_BrokerConsole):
    ...
    def do_status(self,args):
        ...
        if not self.obj.initialized:
            ...
            lines.append(" ")   # just a placeholder for now in case we want to put stuff back
        for line in lines:
            print(("|    {line:74}|".format(line=line)))
        print(("-")*80)
    def do_report(self,args):
