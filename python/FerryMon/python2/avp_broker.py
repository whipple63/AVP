...
class FlowBroker(_BrokerClient):
    '''
    Adds methods and attributes specific to the flow sensor
    Public Methods: None
    Instance Variables: broker_name
    '''
    BROKER_NAME = 'flow'
    def __init__(self,config,**kwargs):
        ...
        self.logger = logging.getLogger(self.__class__.__name__)
        self.load_config(reload_config=False,debug_mode=debug_mode)
        super(FlowBroker, self).__init__(self.config,self.BROKER_NAME,**kwargs) # Run Superclass __init__
    def load_config(self,reload_config=False,**kwargs):
        '''
        Set variables based on self.config. Allows changes to avp.ini to be read in.
        ...
        if reload_config is True:
            self.config.reload()
        # ...doesn't have anything to load.
        super(FlowBroker, self).load_config(reload_config=reload_config,debug_mode=debug_mode) # Run Superclass load_config
        return
    def water_on(self,**kwargs):
        ''' water_on is a method specific to the flow broker.
        '''
        if not self._is_initialized(method_name='water_on'): return {}
        self._json_id += 1
        params = { "cmd" : "water_on", "nLineReply" : 3 }
        w_result =  self.socket_handler.send_rpc('send_command',
                                                    self._json_id,
                                                    timeout=10,
                                                    params=params,
                                                    **kwargs)
        return self._result_checker(w_result)
    def water_off(self,**kwargs):
        ''' water_off is a method specific to the flow broker.
        '''
        if not self._is_initialized(method_name='water_off'): return {}
        self._json_id += 1
        params = { "cmd" : "water_off", "nLineReply" : 3 }
        w_result =  self.socket_handler.send_rpc('send_command',
                                                    self._json_id,
                                                    timeout=10,
                                                    params=params,
