...
class CastDB(AvpDB):
    '''
    Extends db Class with methods specific to an AVP cast
    '''
    def __init__(self, config):
        self.logger = logging.getLogger(self.__class__.__name__)
        try:
            ...
            self.logger.critical('Error in CastDB.__init__() finding configuration key '+str(e))
            sys.exit(1)
        AvpDB.__init__(self,config,self.table) # Run superclass _init__ 
        self.cast_items = {}  # This will hold the fields and values to be inserted or updated.
    def cast_number(self,**kwargs):
        debug_mode = kwargs.get('debug_mode',False) 
        result = self.select(('max(cast_no)',),fetch_type='one',**kwargs)[0]
        if debug_mode: print("cast_number result:{0}".format(result))
        return  {'result':result}
    def start(self,cast_number,**kwargs):
        '''
        Called at start of sonde lowering, Updates record (cast time)
        '''
        debug_mode = kwargs.get('debug_mode',False) 
        set_values = {'cast_time':datetime.now(pytz.reference.LocalTimezone())}
        where_condition = {'cast_no':cast_number}
        self.update(set_values,
                    where_condition=where_condition,
                    **kwargs) #returns cast_number
        #self.commit()
        return
    def bottom_strike(self,cast_number,**kwargs):
        '''
        Called if we have a bottom strike during a cast
        '''
        debug_mode = kwargs.get('debug_mode',False) 
        set_values = {'bottom_strike':True}
        where_condition = {'cast_no':cast_number}
        self.update(set_values,
                    where_condition=where_condition,
                    **kwargs)
