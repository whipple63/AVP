...
class Cast(object):
    ...
    def check_depth(self,**kwargs):
        ...
        if result['01 max_depth'] > self.winch.CABLE_LENGTH:
            result['01 max_depth'] = self.winch.CABLE_LENGTH
            self.logger.info("Limiting cast depth to {1}m based on cable length.".format(self.depth_target,result['01 max_depth']))
        if result['01 max_depth'] > self.winch.MAX_CAST_DEPTH:
            result['01 max_depth'] = self.winch.MAX_CAST_DEPTH
            self.logger.info("Limiting cast depth to {1}m based on MAX_CAST_DEPTH setting.".format(self.depth_target,result['01 max_depth']))
        if self.depth_target > result['01 max_depth']:
            ...
            self.depth_target = result['01 max_depth']
            #self.logger.info("Using DEFAULT DEPTH target of {0}m. ".format(self.depth_target,DEFAULT_DEPTH))
