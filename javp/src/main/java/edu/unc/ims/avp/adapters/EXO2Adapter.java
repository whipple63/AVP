...
import edu.unc.ims.instruments.ysi_exo.EXO2;
import edu.unc.ims.avp.Logger.LogLevel;
import edu.unc.ims.instruments.ysi_exo.EXO2Data;
import edu.unc.ims.instruments.ysi_exo.EXO2Listener;
 ...
 */
public class EXO2Adapter extends BrokerAdapter implements EXO2Listener, Runnable {

    private EXO2 mSonde;
    private EXO2Data mLastReading = null;
    private Date mLastReadingTimestamp;
    private double mDepthOffset;
     ...
     * @param broker 
     */
    public EXO2Adapter(final Broker broker) {
        mBroker = broker;
    ...
    public final void connect() throws Exception {
        ...
        mSonde = new EXO2(mBroker, mBroker.getProperties().getProperty(mBroker.getAdapterName() + "_host", "localhost"),
    ...
    public void extendedMethod(String method, JSONObject request, BrokeredDeviceListener listener) throws Exception {
        ...
        try {
            ...
            if(method.equals("wipe")) {
                ...
                // Because calibration is not avilable with the DCP 2, we will read the current
                // pressure and use it as an offset so that the current value is zero.  It is the 
                // callers responsibility to insure that the sonde is out of water when this is
                // called (presuming you want the zero to be out of water).
                // is the sonde sampling
                if (!getSampling()) {   // if it is not sampling, start it
                    setSampling(true);  // turn on sampling
                    
                    Thread.sleep(5000);     // wait for readings to stabilize (in case run mode just started)
                    mDepthOffset = mLastReading.getDouble("depth_m");   // save the pressure value
                    setSampling(false); // turn sampling back off
                } else {
                    Thread.sleep(5000);     // wait for readings to stabilize (in case run mode just started)
                    mDepthOffset = mLastReading.getDouble("depth_m");   // save the pressure value
                }
    ...
    public void get(HashMap<String, Object> params, BrokerMessage reply) throws Exception {
        ...
        while (iter.hasNext()) {
            ...
            if (ps.equals("filter")) { newValue = mSonde.getFilter();             timeVal = mSonde.getConnectTime(); }
                    ...
                    else if (ps.equals("depth_m")) { 
                        newValue = mLastReading.getDouble(ps) - mDepthOffset;
     ...
     */
    @Override
    public final void onMeasurement(final EXO2Data d) {
...
//            SimpleDateFormat format = new SimpleDateFormat("Mdyy Hms");
            Date tmpDate =  format.parse(d.get("date_mdy") + " " + d.get("time_hms"));
            // the sonde time only reads in hours minutes and seconds (no fractions),
            // so higher than 1 hz sampling rates or duplicate time stamps are impossible
            // because the database will reject them.  Also, the exo sonde has a bug
            // where every once in a while at 1 hz it gives the same time stamp twice in
            // a row
            if (tmpDate.equals(mLastReadingTimestamp)) {
//                tmpDate.setTime( tmpDate.getTime() + 500 ); // add 500 ms to handle duplicate time stamps
                // skip duplicate time stamped data
                return; // get out without inserting data
            }
    ...
    protected final void insertData() {
        ...
        pstmt.setDouble(9, mLastReading.getDouble("depth_m")-mDepthOffset);

        if (mLastReading.get("turbidity_FNU") != null) {
            pstmt.setDouble(10, mLastReading.getDouble("turbidity_FNU"));
        }

        // values that are missing come out as NAN
...
//        }
    ...
    private void setupJSON() {
        List<EXO2Data.ElementDesc> d = EXO2Data.getAvailableDataElements();
        Iterator<EXO2Data.ElementDesc> i = d.iterator();
        while(i.hasNext()) {
            EXO2Data.ElementDesc e = i.next();
